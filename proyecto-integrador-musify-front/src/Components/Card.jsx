  ...
  return (
    ...
    <div className="w-full max-w-sm h-96 bg-white border border-orange-300 rounded-lg flex flex-col items-center ">
      ...
      <div className='w-full h-2/3'>
        <Link to={`/details/${id}`}>
        {images.length > 0 && <img className="rounded-t-lg w-full h-2/3" src={images[0].imageUrl} alt="imagen del producto" />}

      </Link></div>
      <div className="h-1/4">
        <Link to="/details">
          <h5 className="text-base font-semibold tracking-tight text-gray-900 dark:text-white">{name}</h5>
        </Link>
        ...
        <div className="flex items-center flex-col h-1/4 mt-5">
